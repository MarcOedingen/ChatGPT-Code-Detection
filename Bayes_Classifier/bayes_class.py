...
def classify_tokenized_code(tokenized_code, tokens_above_tau, prob_tk_given_H, prob_tk_given_G, P_H=0.5, P_G=0.5):
    filtered_tokens = [token for token in tokenized_code if token in tokens_above_tau]

    log_prob_sum_H = np.sum([np.log(prob_tk_given_H[token]) for token in filtered_tokens])
    log_prob_sum_G = np.sum([np.log(prob_tk_given_G[token]) for token in filtered_tokens])
...
def run_on_problems(code_data, seed):
    ...
    tokens_above_tau = set(human_counter_tau.keys()).intersection(set(gpt_counter_tau.keys()))
    total_human_tokens = sum(human_counter_tau.values())
    total_gpt_tokens = sum(gpt_counter_tau.values())
    prob_tk_given_H = {token: human_counter_tau[token] / total_human_tokens for token in tokens_above_tau}
    prob_tk_given_G = {token: gpt_counter_tau[token] / total_gpt_tokens for token in tokens_above_tau}

    predictions = tokenized_test.apply(
        lambda x: classify_tokenized_code(x, tokens_above_tau, prob_tk_given_H, prob_tk_given_G)
    )

    y_test = X_test['label']
    y_pred = predictions
    y_prob = predictions
    utils.save_results(y_test=y_test, y_pred=y_pred, y_prob=y_prob, file_name="Bayes_Classifier")
